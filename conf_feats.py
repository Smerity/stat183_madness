...
def get_gameconffeats_for_seasons():
  all_teams = set()

  # Create features for training and testing data
  def create_gameconffeats(season, teamA, teamB):
    f = {}
  ...
  with open('temp/conffeatures_data.csv', 'rb') as feature_file:
  ...
  for season in seasons:
    ...
    for i in xrange(len(teams) - 1):
      ...
      for j in xrange(i + 1, len(teams)):
        teamB = teams[j]
        gameconffeats[(teamA, teamB)] = create_gameconffeats(season, teamA, teamB)
    #
    gameconf_features[season] = gameconffeats
  return gameconf_features
...
def get_gameconffeats():
  print 'Retrieving features...'
  sgameconffeats = get_gameconffeats_for_seasons()
  ...
  def produce_csv(fn, season_list, ONLY_TRAINING, ONLY_TEAMS=None):
    ...
    for s in season_list:
      ...
      for k in sorted(sgameconffeats[s]):
        ...
        #
        if rownum == 0:
          FEATURES = sorted(gameconffeats.keys())
          HEADER = ['match'] + FEATURES
          f.write(', '.join('"' + str(h) + '"' for h in HEADER) + '\n')
        name = '{}_{}_{}'.format('"' + s, teamA, teamB + '"')
        fline = ', '.join('"' + str(gameconffeats[f]) + '"' for f in FEATURES)
        f.write('{}, {}\n'.format(name, fline))
        rownum += 1
    f.close()
  #
  print 'Creating train.csv...'
  produce_csv('temp/train_gameconf.csv', train_seasons, ONLY_TRAINING=True)
  ...
  produce_csv('temp/test_gameconf.csv', test_seasons, ONLY_TRAINING=False, ONLY_TEAMS=only_teams)
