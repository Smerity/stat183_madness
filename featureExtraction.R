...
season.mean <- aggregate(cbind(win, score, opp.score) ~ id + name + season + season.id,
                         season.data, FUN=mean)
colnames(season.mean) <- c("id", "name", "season", "season.id", "mean_seas.win", "mean_seas.score", "mean_seas.opp.score")

# Tourney aggregates by teams and seasons
tourney.mean <- aggregate(cbind(win, score, opp.score) ~ id + name + season + season.id,
                          tourney.data, FUN=mean)
colnames(tourney.mean) <- c("id", "name", "season", "season.id", "mean_tourn.win", "mean_tourn.score", "mean_tourn.opp.score")
...
metric.mean <- aggregate(cbind(chess.orank, WP, OWP, OOWP, RPI, SOS, RPI_orank, SOS_orank) ~ id + name +
                          season + season.id, metric.data, FUN=mean)
colnames(metric.mean) <- c("id", "name", "season", "season.id", "mean_chess.orank", "mean_WP", "mean_OWP",
                           "mean_OOWP", "mean_WPI", "mean_SOS", "mean_RPI_orank", "mean_SOS_orank")
# feat.data<-merge(feat.data, metric.mean, all=TRUE)
rm(metric.mean)

metric.min <- aggregate(cbind(chess.orank, WP, OWP, OOWP, RPI, SOS, RPI_orank, SOS_orank) ~ id + name +
                          season + season.id, metric.data, FUN=min)
colnames(metric.min) <- c("id", "name", "season", "season.id", "min_chess.orank", "min_WP", "min_OWP",
                           "min_OOWP", "min_WPI", "min_SOS", "min_RPI_orank", "min_SOS_orank")
# feat.data<-merge(feat.data, metric.min, all=TRUE)

metric.max <- aggregate(cbind(chess.orank, WP, OWP, OOWP, RPI, SOS, RPI_orank, SOS_orank) ~ id + name + 
                          season + season.id, metric.data, FUN=max)
colnames(metric.max) <- c("id", "name", "season", "season.id", "max_chess.orank", "max_WP", "max_OWP",
                           "max_OOWP", "max_WPI", "max_SOS", "max_RPI_orank", "max_SOS_orank")
keep.max <- c("id", "name", "season", "season.id", "max_chess.orank", "max_RPI_orank")
feat.data<-merge(feat.data, metric.max[, ((names(metric.max) %in% keep.max))], all=TRUE)

metric.var <- data.frame(cbind(metric.max[, 1:4], metric.max[, 5:12] - metric.min[, 5:12]))
colnames(metric.var) <- c("id", "name", "season", "season.id", "var_chess.orank", "var_WP", "var_OWP",
...
metric.grad <- aggregate(cbind(chess.orank, WP, OWP, OOWP, RPI, SOS, RPI_orank, SOS_orank) ~ id + name + 
                           season + season.id, metric.data, FUN=Gradient)
colnames(metric.grad) <- c("id", "name", "season", "season.id", "grad_chess.orank", "grad_WP", "grad_OWP",
                           "grad_OOWP", "grad_WPI", "grad_SOS", "grad_RPI_orank", "grad_SOS_orank")
keep.grad <- c("id", "name", "season", "season.id", "grad_chess.orank", "grad_RPI_orank")
